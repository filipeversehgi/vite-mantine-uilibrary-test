...
import { HFPasswordInputProps, PasswordRequirementsInput, PasswordRequirementsProps } from './PasswordInput.types'
...
function HFPasswordInput<T extends FieldValues>({
    ...
    const checks = defaultPasswordRequirements.map((requirement, index) => (
        <PasswordRequirements key={index} label={requirement.label} meets={requirement.re.test(value)} />
    ))
