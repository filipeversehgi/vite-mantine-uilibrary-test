...
export function useContextMenu() {
    ...
    const openContextMenu = (event: MouseEvent): void => {
        ...
        const menuYPosition = height - MENU_HEIGHT > event.clientY ? event.clientY : event.clientY - MENU_HEIGHT
    ...
    const ContextMenu = ({ children }: { children: ReactNode }) => (
        <>
            ...
            {contextMenuOpened && (
                <Overlay style={{ opacity: 0 }} zIndex={90} fixed onClick={() => setContextMenuOpened(false)} />
            )}
        </>
    )
